...
export const SignInWrapper = styled(View)`
  ...
  background-color: ${({
    theme,
  } : ThemedStyledProps<unknown, IRNTheme>) : string => theme.colors.lightGrey};
  flex: 1;
...
export const OAuthHeading = styled(Text)`
  color: ${({
    theme,
  } : ThemedStyledProps<unknown, IRNTheme>) : string => theme.colors.black};
  font-family: ${({
    theme,
  } : ThemedStyledProps<unknown, IRNTheme>) : string => theme.typography.fontFamily};
  font-size: ${({
    theme,
